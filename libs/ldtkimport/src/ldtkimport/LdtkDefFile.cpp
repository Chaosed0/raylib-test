   ...
   const char *ldtkText, size_t textLength, bool loadDeactivatedContent, const char *filename)
{
   ...
   yyjson_arr_foreach(levels, levelIdx, levelLen, level)
   {
      ...
      yyjson_arr_foreach(layerInstances, layerInstanceIdx, layerInstanceLen, layerInstance)
      {
         ...
         auto intGridCsv = yyjson_obj_get(layerInstance, "intGridCsv");
         if (yyjson_is_null(intGridCsv))
         {
             continue;
         }

         auto cwidJson = yyjson_obj_get(layerInstance, "__cWid");
         auto cheiJson = yyjson_obj_get(layerInstance, "__cHei");
		 if (yyjson_is_null(cwidJson) || yyjson_is_null(cheiJson))
         {
             continue;
         }

         dimensions_t cwid = yyjson_get_int(cwidJson);
         dimensions_t chei = yyjson_get_int(cwidJson);

         std::vector<intgridvalue_t> intGridCsvNums(cwid * chei);

		 size_t intGridCsvIdx, intGridCsvLen;
         yyjson_val *intGridCsvVal;
         yyjson_arr_foreach(intGridCsv, intGridCsvIdx, intGridCsvLen, intGridCsvVal)
         {
             int val = yyjson_get_int(intGridCsvVal);
             intGridCsvNums[intGridCsvIdx] = val;
         }

         level = Level();
         level->setIntGrid(cwid, chei, std::move(intGridCsvNums));
      }

      if (level.has_value())
      {
          m_levels.push_back(level.value());
      }
   }
